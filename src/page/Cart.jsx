...
const Cart = () => {
  ...
  // const scaleform = useAnimatedStyle(() => {
  //   return {
  ...
  const handler = () => {
    console.log('Hallo')
    if (xaxis.value == 0) {
      xaxis.value = withTiming(183, { duration: 1100 });
      yaxis.value = withTiming(-706, { duration: 1100 });
      scale.value = 1;

      setTimeout(() => {
        xaxis.value = withTiming(0, { duration: 1500 })
        yaxis.value = withTiming(0, { duration: 1500 })
        scale.value = 0
        setCount(count + 1)
        setTimeout(() => {
